...
When used
- Building a role/group baseline for a department/team OU (audit before onboarding changes).
...
- Confirm the OU scope matches what you intended (OneLevel vs Subtree).

What was sanitized
- Internal DC hostnames, OU DNs, and usernames removed (now parameters/placeholders).
- Original local filename: `Operations.ps1`.
#>
...
param(
  ...
  [string]$OutputCsvPath = (Join-Path -Path (Get-Location) -ChildPath 'operations_user_groups.csv')
)
